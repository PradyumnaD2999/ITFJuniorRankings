module.exports = async function (input) {
    ...
    let apiSearchUrl = `https://www.itftennis.com/tennis/api/PlayerApi/GetPlayerSearch?searchString=${input.name}`;
    console.log('API Search URL: ', apiSearchUrl);
    let playerSearchJson = await extractorContext.fetch(apiSearchUrl, {
            headers: {
    ...
    if(playerSearchJson.players.length) {
        console.log('Fetched Player List.');
        for(let i = 0; i < playerSearchJson.players.length; i++) {
            console.log('i: ', i);
            if(playerSearchJson.players[i].playedCircuits.length) {
                console.log('Fetched Played Circuits List.');
                for(let j = 0; j < playerSearchJson.players[i].playedCircuits.length; j++) {
                    console.log('j: ', j);
                    if(playerSearchJson.players[i].playedCircuits[j].label.toLowerCase() == 'juniors' || playerSearchJson.players[i].playedCircuits[j].value.toLowerCase() == 'jt') {
                        console.log('Junior Tournament Profile Found.');
                        let searchUrl = playerSearchJson.players[i].playerProfileLink;
                        let finalSearchUrl = `https://www.itftennis.com${searchUrl}jt`;
                        console.log('Final Search URL: ', finalSearchUrl);
                        await extractorContext.goto(finalSearchUrl);
                        ...
                        let overviewUrl = `https://www.itftennis.com/tennis/api/PlayerApi/GetPlayerOverview?circuitCode=JT&matchTypeCode=S&playerId=${playerSearchJson.players[i].playerId}`
                        ...
                        let activityUrl = `https://www.itftennis.com/tennis/api/PlayerApi/GetPlayerActivity?circuitCode=JT&matchTypeCode=S&playerId=${playerSearchJson.players[i].playerId}`;
                        ...
                        if(tournamentList.length) {
                            for(let k = 0; k < tournamentList.length; k++) {
                                ...
                                if(!tournamentList[k].tournamentType.toLowerCase().includes('grade c')) {
                                    ...
                                    playerData.proPoints = input.points ? input.points : '';
                            ...
                            playerDetails.proPoints = input.points ? input.points : '';
                        }
                        ...
                        console.log('Player Juniors Tournament Profile Not Present.');

                        playerDetails.detailsUrl = '';
                        playerDetails.name = input.name ? input.name : '';
                        playerDetails.ranking = input.ranking ? input.ranking : '';
                        playerDetails.rankingYear = input.rankingYear ? input.rankingYear : '';
                        playerDetails.yob = input.yob ? input.yob : '';
                ...
                console.log('Player hasn\'t played any circuits.');
        ...
        console.log('Player Profile Not Present.');
